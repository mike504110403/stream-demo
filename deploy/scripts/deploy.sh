...
# 檢查環境變數文件
check_env_file() {
    local env_file=${1:-"deploy/env/.env"}
    
    if [ ! -f "$env_file" ]; then
        log_warning "環境變數文件 $env_file 不存在"
        log_info "正在複製範例文件..."
        if [ -f "deploy/env/.env.example" ]; then
cp deploy/env/.env.example "$env_file"
...
backup_data() {
    ...
    # 備份配置
    log_info "備份配置..."
    tar -czf "$backup_dir/config_backup.tar.gz" deploy/ infrastructure/ services/
